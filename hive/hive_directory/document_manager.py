...
class DocumentManager:
    ...
    def _initialize_subdirectories(self) -> None:
        ...
        """
        # サブディレクトリを作成
        for subdir in [self.report_dir, self.analysis_dir, self.design_dir, self.meeting_dir]:
    ...
    def cleanup_old_documents(self, older_than_days: int) -> None:
        ...
        try:
            ...
            for doc_dir in [self.report_dir, self.analysis_dir, self.design_dir, self.meeting_dir]:
                if doc_dir.exists():
            ...
            logger.info(f"Cleaned up {deleted_count} old documents (older than {older_than_days} days)")
